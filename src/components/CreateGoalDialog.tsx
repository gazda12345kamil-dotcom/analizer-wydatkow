...
export function CreateGoalDialog({ open, onOpenChange, editGoal }: CreateGoalDialogProps) {
  ...
  // Wypełnij formularz wartościami celu do edycji
  useEffect(() => {
    if (open && editGoal) {
      form.reset({
        name: editGoal.name,
        targetAmount: editGoal.targetAmount,
        color: editGoal.color || COLORS[7],
        icon: editGoal.icon || ICONS[0],
        deadline: editGoal.deadline ? new Date(editGoal.deadline) : undefined,
      })
    } else if (open && !editGoal) {
      form.reset({
        name: "",
        targetAmount: 0,
        color: COLORS[7],
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      if (isEditMode && editGoal) {
        ...
        await db.saving_goals.update(editGoal.id, {
          name: values.name,
        ...
        {isSuccess ? (
           <div className="flex flex-col items-center justify-center p-12 text-center h-[400px]">
             ...
             <h3 className="text-2xl font-black text-slate-800 tracking-tight">
               {isEditMode ? 'Zapisano!' : 'Gotowe!'}
             </h3>
             <p className="text-slate-500 font-medium mt-2">
               {isEditMode ? 'Cel został zaktualizowany.' : 'Skarbonka została pomyślnie otworzona. Zacznij oszczędzać.'}
             </p>
           </div>
        ) : (
          <>
            <DialogHeader className="p-6 pb-4 border-b border-slate-100 bg-slate-50/50">
              <DialogTitle className="text-xl font-bold flex items-center gap-2">
                ...
                {isEditMode ? 'Edytuj Cel' : 'Załóż nową Skarbonkę'}
              </DialogTitle>
              <DialogDescription className="font-medium text-slate-500">
                {isEditMode 
                  ? 'Zaktualizuj nazwę, kwotę docelową lub termin.'
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-5">
                    ...
                    render={({ field }) => (
                      <FormItem className="flex-[1.2]">
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-auto p-0 bg-white border-slate-200 shadow-xl rounded-2xl overflow-hidden z-[100]" align="end">
                            <Calendar
                ...
                <div className="pt-4">
                  ...
                  >
                    {isSubmitting ? <><Loader2 className="mr-2 h-6 w-6 animate-spin" /> Zapisywanie...</> : isEditMode ? "Zapisz zmiany" : "Stwórz Cel"}
                  </Button>
                </div>
