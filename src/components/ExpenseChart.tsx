...
const renderActiveShapeMobile = (props: any) => {
  ...
  const shortName = payload.name.length > 8 ? payload.name.slice(0, 8) + '…' : payload.name;
  return (
    <g>
      <text x={cx} y={cy - 8} textAnchor="middle" fill={fill} fontSize={10} fontWeight="bold">
        {shortName}
      </text>
      <text x={cx} y={cy + 5} textAnchor="middle" fill="#334155" fontSize={10} fontWeight={700}>
        {`${Number(value).toLocaleString('pl-PL')} zł`}
      </text>
      <text x={cx} y={cy + 18} textAnchor="middle" fill="#94a3b8" fontSize={9} fontWeight={500}>
      ...
      <Sector cx={cx} cy={cy} startAngle={startAngle} endAngle={endAngle} innerRadius={outerRadius + 6} outerRadius={outerRadius + 9} fill={fill} />
    </g>
  );
};
...
const renderActiveShapeDesktop = (props: any) => {
  ...
  return (
    <g>
      <text x={cx} y={cy - 4} textAnchor="middle" fill={fill} fontSize={12} fontWeight="bold">
      ...
      <Sector cx={cx} cy={cy} startAngle={startAngle} endAngle={endAngle} innerRadius={outerRadius + 7} outerRadius={outerRadius + 10} fill={fill} />
