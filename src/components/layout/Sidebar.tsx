...
import { Home, PlusCircle, PieChart, Wallet, Sparkles, Menu, PiggyBank, CalendarClock, Settings, X, Trophy, Database } from "lucide-react";
import { cn } from "@/lib/utils";
import { AiReportDialog } from "@/components/AiReportDialog";
import { Sheet, SheetContent, SheetTrigger, SheetTitle } from "@/components/ui/sheet";

const navItems = [
  { icon: Home, label: "Pulpit", href: "/" },
  { icon: PlusCircle, label: "Dodaj wpis", href: "/add" },
  { icon: PieChart, label: "Analiza", href: "/analytics" },
  { icon: PiggyBank, label: "Skarbonki", href: "/saving-goals" },
  { icon: CalendarClock, label: "Subskrypcje", href: "/subscriptions" },
  { icon: Trophy, label: "Osiągnięcia", href: "/achievements" },
  { icon: Database, label: "Dane Bankowe", href: "/data-sync" },
...
export function Sidebar() {
  ...
  const links = [
    ...
    { href: '/data-sync', icon: Database, label: 'Eksport / Import' },
  ];
    ...
    <>
      ...
      <div className="sm:hidden fixed top-0 left-0 right-0 h-16 px-4 flex items-center justify-between bg-white/80 backdrop-blur-xl border-b border-slate-100 z-40 shadow-sm mt-safe">
        <Sheet>
          ...
          <SheetContent side="left" className="w-[85vw] p-0 border-r-0 sm:w-[320px] pb-safe z-50 bg-white/95 backdrop-blur-3xl">
